...
class SDC_Admin {
    ...
    public function init_menus( ) {
        //add_menu_page( 'Sketch', 'Mockups', 'manage_options', 'sdc_main', array( 'SDC_Admin' ,'i_settings'), 'dashicons-align-center', '80.08'  );
    }
    ...
    public function add_sdc_meta_boxes(){
        add_meta_box(
            ...
            esc_html__( 'Structured Data Scheme', '' ),		// Title
    ...
    public function sdc_actions( ) {
        ...
        if ( isset( $_POST['sdc_action'] )) {
            ...
            if( $_POST['sdc_action'] == 'update_mockup' ) {
                if( $_POST['sdc_action']['attachment_id'] == '' ){
                    return false;
                }
                $sdc_uploads = $_POST['sdc_uploads'];
                update_option( 'sdc_uploads', $sdc_uploads );
            }
    ...
    public function i_settings( ){
        /*self::sdc_actions();
        wp_enqueue_media();
        require_once ( SDC_PLUGIN_DIR . 'view/admin/sdc_main.php' );*/
    }
    ...
    function create_section_for_image_url( $field, $value = '', $attrs = '' ) {
        ...
        $html.= '<img src="'.$value.'" class="i_preview_img '.$class.'" id="i_preview_field_'.$field['id'].'_'.$field['id_key'].'" >';
        $html.= '<p class="description">' . $field['description'] . '</p>';
